...
function g_msg_handler.c2s_role_enter_req(data, mailbox_id)
	...
	if not user then
		...
		net_mgr:send_msg(mailbox_id, MID.s2c_role_enter_ret, msg)
		return
	end
	...
	if user._scene_server_id == 0 then
		scene_server_info = server_mgr:get_server_by_scene(scene_id)
	else
		scene_server_info = server_mgr:get_server_by_id(user._scene_server_id)
	end
...
function g_msg_handler.s2s_gate_role_enter_ret(data, mailbox_id)
	...
	local scene_server_info = server_mgr:get_server_by_mailbox(mailbox_id)
	if not scene_server_info then
...
function g_msg_handler.transfer_msg(mailbox_id, msg_id, ext)
	...
	if ext == 0 then
		...
		local scene_server_info = server_mgr:get_server_by_id(scene_server_id)
		if not scene_server_info then
