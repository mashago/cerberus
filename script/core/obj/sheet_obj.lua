...
function SheetObj:load_data()
	...
	local status, ret = rpc_mgr:call_by_server_type(ServerType.DB, "db_game_select", rpc_data, self._keys[1][3])
	if not status then
...
function SheetObj:do_save(insert_rows, delete_rows, modify_rows)
	...
	if #insert_rows then
		...
		rpc_mgr:send_by_server_type(ServerType.DB, "db_game_insert_multi", rpc_data)
	end
	...
	if #delete_rows then
		...
		rpc_mgr:send_by_server_type(ServerType.DB, "db_game_delete_multi", rpc_data)
	end
	...
	if #modify_rows then
		...
		rpc_mgr:send_by_server_type(ServerType.DB, "db_game_update_multi", rpc_data)
	end
