...
function ServiceServerInfo:ctor(ip, port, server_id, server_type, register, no_reconnect)
	self._ip = ip 
	self._port = port 
...
function ServiceServerInfo:print()
	...
	Log.info("register=%d no_reconnect=%d", self._register, self._no_reconnect)
