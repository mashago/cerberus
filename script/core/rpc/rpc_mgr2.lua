...
function RpcMgr:ret(data)
	...
	local running_env = self._running_env
	assert(running_env, "RpcMgr:ret running_env nil")
	assert(running_env.session_id ~= RPC_SEND_SESSION_ID 
	and running_env.session_id ~= RPC_RET_SESSION_ID
	, "RpcMgr:ret error " .. running_env.session_id)

	local msg =
	{
		result = true, 
		from_server_id = running_env.from_server_id, 
		to_server_id = running_env.to_server_id, 
		session_id = running_env.session_id, 
		param = Util.serialize(data)
	}
	Core.net_mgr:send_msg(running_env.mailbox_id, MID.s2s_rpc_ret, msg)

	-- mark already ret
	running_env.session_id = RPC_RET_SESSION_ID
end
...
function RpcMgr:handle_call(data, mailbox_id, msg_id, is_send)
	...
	self._running_env = 
	{
		...
		from_server_id = data.from_server_id,
		to_server_id = data.to_server_id,
		session_id = data.session_id,
	}
	local running_env = self._running_env

	local function send_error(errno)
		if running_env.session_id == RPC_RET_SESSION_ID then
			-- already ret
			Log.warn("RpcMgr:handle_call send_error already ret")
			return
		end
		local msg =
		{
	...
	-- miss ret
	Log.err("RpcMgr:handle_call miss ret func_name=%s", func_name)
	send_error(-1)
	return
end
