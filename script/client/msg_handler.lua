...
function g_msg_handler.s2c_role_list_ret(data, mailbox_id, msg_id)
	...
	Env.g_client._area_role_list = data.area_role_list
	Env.g_time_counter:print()
end
...
function g_msg_handler.s2c_select_role_ret(data, mailbox_id, msg_id)
	...
	Env.g_client._server_list[ServerType.GATE] =
	{
		ip = data.ip,
		port = data.port,
	...
	Env.g_client._user_id = data.user_id
	Env.g_client._user_token = data.token
...
function g_msg_handler.s2c_role_attr_ret(data, mailbox_id, msg_id)
	...
	Env.g_role = Role.new(role_id)
	Env.g_role:init_data(attr_table)
	Env.g_role:print()
end
...
function g_msg_handler.s2c_role_attr_change_ret(data, mailbox_id, msg_id)
	...
	local attr_table = data.attr_table
	if Env.g_role then
		Env.g_role:update_data(attr_table)
		Env.g_role:print()
	end
end
...
function g_msg_handler.s2c_attr_info_ret(data, mailbox_id, msg_id)
	...
	if sheet_name == "role_info" then
		...
		Env.g_role:init_data(data.rows[1])
		Env.g_role:print()
	end
end
...
function g_msg_handler.s2c_attr_modify_ret(data, mailbox_id, msg_id)
	...
	if sheet_name == "role_info" then
		...
		Env.g_role:update_data(data.rows[1].attrs)
		Env.g_role:print()
	end
